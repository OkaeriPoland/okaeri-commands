...
import static eu.okaeri.commands.bukkit.CommandsBukkit.INVOKE_SYNC_WARN_TIME;

@RequiredArgsConstructor
public class BukkitSchedulingHandler implements SchedulingHandler {

    protected final Map<Method, Boolean> isAsyncCacheMethod = new ConcurrentHashMap<>();
    protected final Map<Class<? extends CommandService>, Boolean> isAsyncCacheService = new ConcurrentHashMap<>();
    ...
    public void run(@NonNull CommandData data, @NonNull Invocation invocation, @NonNull Runnable runnable) {
        ...
        if (this.isAsync(invocation)) {
            this.runAsync(data, invocation, runnable);
            return;
        }

        Instant start = Instant.now();
        this.runSync(data, invocation, runnable);
